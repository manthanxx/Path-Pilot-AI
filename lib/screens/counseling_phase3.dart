...
class _CounselingPhase3State extends State<CounselingPhase3> {
  ...
  final List<String> budgetRanges = ["Free only", "Low cost", "Medium", "Any budget"];
          ...
          child: Column(
            ...
            children: [
              ...
              Expanded(
                child: ListView(
                  children: [
                    ...workStyles.map((style) => _buildWorkStyleCard(style)).toList(),
                    ...
                    Wrap(
                      ...
                      children: budgetRanges.map((budget) {
                        ...
                        return ChoiceChip(
                          label: Text(budget),
                          selected: isSelected,
                          onSelected: (val) => setState(() => selectedBudget = val ? budget : null),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: (selectedWorkStyle == null || selectedBudget == null)
                      ? null
                      : () => Navigator.pushNamed(context, '/counseling_phase4'),
  ...
  Widget _buildWorkStyleCard(Map<String, dynamic> style) {
    ...
    return GestureDetector(
      onTap: () => setState(() => selectedWorkStyle = style['title']),
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 200),
        margin: const EdgeInsets.only(bottom: 12),
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: isSelected ? Colors.blueAccent.withOpacity(0.05) : Colors.grey.shade50,
          borderRadius: BorderRadius.circular(18),
          border: Border.all(color: isSelected ? Colors.blueAccent : Colors.transparent, width: 2),
        ),
        child: Row(
          children: [
            Icon(style['icon'], color: isSelected ? Colors.blueAccent : Colors.blueGrey, size: 28),
            const SizedBox(width: 16),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(style['title'], style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: isSelected ? Colors.blueAccent : Colors.black87)),
                Text(style['desc'], style: TextStyle(fontSize: 14, color: Colors.grey.shade600)),
              ],
            ),
            const Spacer(),
            if (isSelected) const Icon(Icons.check_circle_rounded, color: Colors.blueAccent),
          ],
        ),
      ),
    );
